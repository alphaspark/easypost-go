...
func (c *Client) NewShipmentWithExistingAddress(fromAddressId string, toAddressId string, parcel Parcel) (s Shipment, e error) {
	shipmentBody := map[string]interface{}{
		"shipment": Shipment{
			"from_address": Address{"id": fromAddressId},
			"to_address":   Address{"id": toAddressId},
			"parcel":       parcel,
		},
	}
	// Encode JSON body
...
func (c *Client) BuyShipmentRate(shipmentId string, rateId string) (s Shipment, e error) {
	shipmentBody := map[string]interface{}{
		"rate": Rate{
			"id": rateId,
		},
	...
	// POST to /shipments
	req, rErr := http.NewRequest("POST", fmt.Sprintf("%s%s/%s/buy", c.baseURL, "/shipments", shipmentId), bytes.NewBuffer(jsonStr))
	if rErr != nil {
		return nil, rErr
	}
	req.Header.Set("Content-Type", "application/json")
	...
	return nil, errors.New(fmt.Sprintf("Problem buying rate for shipment: %s", body))
}
